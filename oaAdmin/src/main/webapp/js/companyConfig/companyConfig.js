function deletea(tar){
	JEND.page.confirm('配置删除后不可恢复,确认要删除该配置？', function() {
		
		JEND.page.dialog.show({ title:'删除配置', id:'deleteWin', width:800, height:700 });
		//加载要删除的ID
		var deleteID = $(tar).attr('id');
		$("#deleteID").val(deleteID);
	}, function() {});
}

function recover(tar){
		$.ajax({
			async:false,
			type:'post',
			dataType:'json',
			data:{'id':$(tar).attr('id')},
			url:contextPath + "/companyConfig/recover.htm",
...
function discard(tar){
	$.ajax({
		async:false,
		type:'post',
		dataType:'json',
		data:{'id':$(tar).attr('id')},
		url:contextPath + "/companyConfig/discard.htm",
		success:function(data){
			$.messager.alert('提示信息', data.message, "info", function() {});
			if(data.success){
				$('#companyConfigTable').datagrid('reload',$("#queryform").serializeParams());
...
$(function() {
	...
	var datagrid=$('#companyConfigTable').datagrid({
		singleSelect : true,
		idField : 'id',
		url : contextPath + "/companyConfig/pageList.htm",
		method : "post",
