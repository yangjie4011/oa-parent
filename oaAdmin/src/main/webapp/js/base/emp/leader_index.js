...
function setVal(row){
	...
	if("1" == addOrEdit){//新增页面
		window.opener.document.getElementById("addLeaderId").value = row.id;
		window.opener.document.getElementById("addLeaderName").value = row.text;
	}else{//编辑页面
		window.opener.document.getElementById("editLeaderId").value = row.id;
		window.opener.document.getElementById("editLeaderName").value = row.text;
	}
