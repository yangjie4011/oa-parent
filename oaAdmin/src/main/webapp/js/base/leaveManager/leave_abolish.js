$(function(){
	...
	$("#export").click(function() {
		...
		window.location.href = basePath + "empApplicationLeaveBack/exportReport.htm?"+data;
	});
...
function gotoPage(page){
	...
	$.ajax({
		...
		dataType:'json',
		data: data,
		url:  basePath + "empApplicationLeaveBack/getReportPageList.htm",
		success:function(response){
			...
			for(var i=0;i<response.rows.length;i++){
				...
				if(response.rows[i].leaveType==5){
					...
					if(response.rows[i].leaveType==7 && response.rows[i].leaveHoursDetail!=null){
						html += "<td style='text-align:center;'>"+getValByUndefined(response.rows[i].leaveDaysDetail-response.rows[i].leaveDays)+"+"+response.rows[i].leaveHoursDetail+"</td>";
					}else{
				...
				if(response.rows[i].leaveType==5){
					...
					if(response.rows[i].leaveType==7 && response.rows[i].leaveHours!=null){
						html += "<td style='text-align:center;'>"+getValByUndefined(response.rows[i].leaveDays+"+"+response.rows[i].leaveHours)+"</td>";
					}else{
...
function pass(){
	...
	passInvalidCancelLeave(processInstanceIds,comment);
...
function refuseAbate(){
	...
	refuseInvalidCancelLeave(processInstanceIds,comment);
	$("#refuseAbateDivForm").find("textarea[name='approvalReason']").val("");
}


function passInvalidCancelLeave(processInstanceIds,comment){
	pageLoading(true);//加载动画
	$.ajax({
 		async : true,
 		type : "post",
		...
		data : {processId:processInstanceIds,comment:comment,token:$("#token").val()},
 		url : contextPath + "/empApplicationLeaveBack/passInvalidCancelLeave.htm",
 		success : function(response) {
 			if(response.sucess){
 				gotoPage($("#pageNo").val());
 			}else{
...
function refuseInvalidCancelLeave(processInstanceIds,comment){
	pageLoading(true);//加载动画
	$.ajax({
 		async : true,
 		type : "post",
		dataType:"json",
		data : {processId:processInstanceIds,comment:comment,token:$("#token").val()},
 		url : contextPath + "/empApplicationLeaveBack/refuseInvalidCancelLeave.htm",
 		success : function(response) {
 			if(response.sucess){
 				gotoPage($("#pageNo").val());
 			}else{
...
function refuseLeave(obj){
	...
	$.ajax({
		...
		data : {processInstanceId:$(obj).attr("processInstanceId"),comment:$("#reason").val(),token:$("#token").val()},
 		url : contextPath + "/empApplicationLeaveBack/refuseLeave.htm",
 		success : function(response) {
 		...
 		success : function(response) {
 			if(response.sucess){
 				...
 				if(leaveType==5){
 					...
 					$("#detailDiv11").html(response.leaveAbolish.leaveDays);
 				}
