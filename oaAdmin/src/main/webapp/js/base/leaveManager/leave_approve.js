...
function gotoPage(page){
	...
	$.ajax({
		...
		dataType:'json',
		data: data,
		url:  basePath + "empApplicationLeave/getApproveIngPageList.htm",
...
function gotoPage1(page){
	...
	$.ajax({
		...
		dataType:'json',
		data: data,
		url:  basePath + "empApplicationLeave/myLeaveTaskList.htm",
		success:function(response){
			$("#reportList1").empty();
			var html = "";
			for(var i=0;i<response.rows.length;i++){
				...
				if(response.rows[i].leaveType==5){
					...
					html += "<td style='text-align:center;'>"+response.rows[i].leaveDays+"天</td>";
				}
				html += "<td style='text-align:center;'>"+response.rows[i].submitDate+"</td>";
				var approvalStatus = "";
				if(response.rows[i].approvalStatus==100){
...
function gotoPage2(page){
	...
	$.ajax({
		...
		dataType:'json',
		data: data,
		url:  basePath + "empApplicationLeave/getAbatePageList.htm",
		success:function(response){
			$("#reportList2").empty();
			var html = "";
			for(var i=0;i<response.rows.length;i++){
...
function getBatchApproveDiv(){
	if($("input[name='selectSingle']:checked").length<=0){
		JEND.page.showError('请选择需要审批的假期申请单！');
		return;
	}
	$("#fastApproveDiv").show();
}
...
function completeLeave(processInstanceIds,comment,commentType){
	...
	$.ajax({
		...
		data : {processIds:processInstanceIds,comment:comment,commentType:commentType,token:$("#token").val()},
 		url : contextPath + "/empApplicationLeave/completeLeave.htm",
 		success : function(response) {
 			if(response.sucess){
 				gotoPage($("#pageNo").val());
 			}else{
