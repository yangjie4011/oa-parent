$(function(){
	...
	$("#export").click(function() {
		...
		window.location.href = basePath + "empAttn/exportMonthLackDetail.htm?"+data;
	});
});
...
function gotoPage(page){
	...
	$.ajax({
		...
		dataType:'json',
		data: data,
		url:  basePath + "empAttn/getMonthLackDetailPageList.htm",
		success:function(response){
			...
			$.each(rows,function(i,item){
				html += "<tr><td>"+formatStr(item.code)+"</td>"
				     ...
				     +  "<td>"+formatStr(item.lateAndEarly)+"</td>"
				     +  "<td>"+formatStr(item.absenteeismTime)+"</td>"
				     +  "<td>"+formatStr(item.lackAttnTime)+"</td></tr>";
			});
