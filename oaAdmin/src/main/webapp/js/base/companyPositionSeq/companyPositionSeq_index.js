...
function savePosition(){
	...
	if(code==""){
    	JEND.page.alert({type:"error", message:"序列编码不能为空"});
    	return;
	}
	...
	if(code==""){
    	JEND.page.alert({type:"error", message:"序列名称不能为空"});
    	return;
	}
		         ...
		         type : "post",  
		          ...
		          success : function(data){  
						if(data==true){
							...
							$("#nameup").val("");
							id=0;
							JEND.page.alert({type:"error", message:"序列名称不能相同!"});
						} 
		          }  
		     }); 
		}	
		         ...
		         type : "post",  
		          ...
		          success : function(data){  
						if(data==true){
							...
							$("#codeup").val("");
							id=0;
							JEND.page.alert({type:"error", message:"序列编码不能相同!"});
						} 
		          }  
		     }); 
		}		
...
function gotoPage(page){
	...
	$.ajax({
		...
		dataType:'json',
		data: data,
		url:  basePath + "companyPositionSeq/getReportPageList.htm",
