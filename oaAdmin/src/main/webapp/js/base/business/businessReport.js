...
function exportReport(){
	var data = encodeURI(encodeURI(decodeURIComponent($("#queryform").serialize(),true)))+"&typeNum="+typeNum+"";
	
	window.location.href = basePath + "/empApplicationBusiness/exportGroupReport.htm?" + data;
}

function gotoPage(page){
	if(typeNum==3){
		if($("#firstCity").val()=="" || $("#secondCity").val()==""){
			JEND.load('util.dialog', function() {
				JEND.util.dialog.alert("请选择地点");
			});
			return;
		}
	}
	...
	$.ajax({
		...
		dataType:'json',
		data: data,
		url:  basePath + "empApplicationBusiness/getGroupListByInfo.htm",
		success:function(response){
			$("#reportList").empty();
			var html = "";
			for(var i=0;i<response.rows.length;i++){
				html += "<tr>";
				html += "<td style='text-align:center;'>"+$("#year").val()+"年</td>";
				...
				html += "<td style='text-align:center;'>"+getValByUndefined(response.rows[i].address)+"</td>";
				html += "<td style='text-align:center;'>"+(getValByUndefined(response.rows[i].peopleNum)==""?0:response.rows[i].peopleNum)+"</td>";
				html += "<td style='text-align:center;'>"+(getValByUndefined(response.rows[i].frequencyNum)==""?0:response.rows[i].frequencyNum)+"</td>";
				html += "<td style='text-align:center;'>"+(getValByUndefined(response.rows[i].duration)==""?0:response.rows[i].duration)+"</td>";			
