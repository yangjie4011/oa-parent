...
public class SmsController {
	...
	public JSON sendRandomCode(String phone) throws OaException, SmsException{
		try{
			String randomCode = smsService.sendRandomCode(phone, "您的验证码为:{password}, 请于30分钟内及时使用【MO办公自动化系统】");
	...
	public JSON sendMessage(String phone,String message) throws OaException, SmsException{
		try{
			logger.info("sms/sendMessage.htm 入参："+JSONUtils.write(phone)+JSONUtils.write(message));
