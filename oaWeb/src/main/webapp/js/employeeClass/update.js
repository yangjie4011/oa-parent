...
function getEmployeeClassSetByMonth(employeeId){
  $.ajaxSettings.async = false;
  $.getJSON(contextPath +'/employeeClass/getEmployeeClassSetByMonth.htm?attnApplicationEmployClassId='+$("#classdetailId").val()+"&employeeId="+employeeId+"&type=1&classMonth="+$("#classMonth").val(),function(data){
...
function initCalendar(employeeId){
	h_dateBag.initDatePage({
        ...
        callback:function(){
        	var flag = true;
        	var classDate = $("#classMonth").val()+"-01";
        	$('#calendarBox .default').each(function(){
        		if($(this).attr("date_day")==$(".on").attr("entrytime")){
        			 $(this).addClass('current');
        			 flag = false;
        			 classDate = $(".on").attr("entrytime");
        			 return;
        		}
        	})
        	if(flag){
        		$('#calendarBox .default:eq(0)').addClass('current');
        	}
            //默认获取一号排班
        	var isMove = 0;
            ...
            $('#calendarBox .default').click(function(event) {
                ...
                bPup(nameArr,idArr,mustAttnTimeArr,startTimeArr,endTimeArr,function(name,id,mustAttnTime,startTime,endTime){
                	...
                	if(classSetId_old!=id){
                		 ...
                		 if(typeof(classSetId_old)!="undefined"&&id!=0){
                			 $(currentObj).addClass('red');
                    	 }
                		 ...
                		 //开始已经有了班次，选择休息之后，需要清除缓存里的数据，并且新增待删除的缓存
                		 if(id==0&&typeof(classSetId_old)!="undefined"){
                			 ...
                			 $.ajax({
                					...
                					url:contextPath + "/employeeClass/removeCache.htm",
                					success:function(data){
                		 ...
                		 if(id!=0){
                			 $.ajax({
               					...
               					url:contextPath + "/employeeClass/cacheEmployeeClassDetail.htm",
               					success:function(data){
	...
	OA.twoSurePop({
		...
		sureFn:function(){
		    	...
		    	$.ajax({
		    		...
		    		url:contextPath + "/employeeClass/saveClassDetail.htm",
		    		success:function(data){
		    			if(data.success){
							//跳页面
