var areaSeled = {
    tpls: {
        z_html: '<div class="z_listBox">\
                    <div class="z_heard">\
                        <span class="fl z_cancel">取消</span>\
                         <span class="rl z_affirm">确定</span>\
                        <p class="z_xzdq">所在地区</p>\
                    </div>\
                    <div class="z_conent">\
                         <span class="z_select z_active">请选择</span>\
                        <span class="z_select1">请选择</span>\
                    </div>\
                    <div class="z_foot">\
                    <ul id="param_province">\
                    </ul>\
                    <ul id="param_city">\
                    </ul>\
                    </div>\
    ...
    //初始化标签
    initDom: function () {
        this.param_province = $('#param_province')
        this.param_city = $('#param_city')
        this.z_select = $('.z_select')
        this.z_select1 = $('.z_select1')
        this.z_listBox = $('.z_listBox')
        this.z_conent = $('.z_conent')
        this.z_text = $('.z_text')
    },
    // 渲染省级列表
    render: function () {
        $('body').append(areaSeled.tpls.z_html)
        this.initDom()
        for (let i = 0; i < areaSeled.data.provinces.length; i++) {
            let province_li = $('<li data_id=' + i + '>' + areaSeled.data.provinces[i] + '</li>')
            this.param_province.append(province_li)
        }
    },
    // 点击事件
    bindEvent: function (cb) {
        var self = this;
        this.z_select.click(function () {
            self.addOrRemore(self.z_conent, self.z_select, 'z_active')
            self.param_city.hide()
            self.param_province.show()
        })
        this.z_select1.click(function () {
            self.addOrRemore(self.z_conent, self.z_select1, 'z_active')
            self.param_province.hide()
            self.param_city.show()
        })
        this.z_text.click(function () {
            self.z_listBox.show()
        })
        //确认点击事件
        $('.z_affirm').click(function () {
//            self.z_select.text() + " " + self.z_select1.text()
            self.z_listBox.hide()
            console.log(self.z_select.text())
            console.log(self.z_select1.text())
            cb && cb({
                province: self.z_select.text(),
                city: self.z_select1.text()
            })
        });
        // 取消点击事件
        ...
        // 省级列表点击事件
        this.param_province.on('click', 'li', function () {
            self.addOrRemore(self.param_province, $(this), 'z_active1')
            let city = self.data.citys[$(this).attr('data_id')]
            self.z_select.text($(this).text())
            self.z_select1.text('请选择')
            self.param_province.hide()
            self.addOrRemore(self.z_conent, self.z_select1, 'z_active')
            self.param_city.html('').show()
            for (let i = 0; i < city.length; i++) {
                let city_li = $('<li data_id=' + i + '>' + city[i] + '</li>')
                self.param_city.append(city_li)
            }
        })
        // 市级列表点击事件
        this.param_city.on('click', 'li', function () {
            self.addOrRemore(self.param_city, $(this), 'z_active1')
            self.z_select1.text($(this).text())
            self.param_city.hide()
        })
    },
    addOrRemore(e1, e2, style) {
        e1.children().removeClass(style)
        e2.addClass(style)
    }
}
