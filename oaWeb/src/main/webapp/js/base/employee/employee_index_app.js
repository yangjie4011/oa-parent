...
//		   dragThis.insertDom();
	...
	$('.selCompanyPartment').click(function () {
	    ...
	    OA.selPart({
	        cb: function (data) {
	            ...
	            $('.selCompanyPartment p').html(data.name);
	...
	$(".three-tab li").click(function(){
		
		if($(this).hasClass("on")){
			return false;
		}
		
		if($(this).hasClass("onTheJob")){
			$("#jobStatus").val("0");
		}else if($(this).hasClass("offTheJob")){
			$("#jobStatus").val("1");
		}else if($(this).hasClass("allTheJob")){
			$("#jobStatus").val("");
		}
		
		$(".three-tab li").removeClass("on");
		$(this).addClass("on");
...
function getEmpList(pageNo, pageSize) {
	...
	$.ajax({
		...
		data : {'pageNo':pageNo,'pageSize':pageSize,'depart.id':$("#partId").val(),
			    'nameOrCode':$("#nameOrCode").val(),'jobStatus':$("#jobStatus").val()},
		dataType : 'json',
...
//        	$("#empList").empty();
			if(response.rows == null || response.rows == "" || response.rows.length < 1) {
				...
				for(var i=0;i<response.rows.length;i++){
					var html = "<div onclick='check("+response.rows[i].id+");'><li>";
					html += "<div>";
					...
					if("" != getValByUndefined(response.rows[i].position.positionName)){
						html += "/" + response.rows[i].position.positionName;
					}
					
					html += "<span>";
					if("" != getValByUndefined(response.rows[i].positionTitle)){
						html += "/" + response.rows[i].positionTitle;
					}
					html += "</span>";
					html += "</div>";
