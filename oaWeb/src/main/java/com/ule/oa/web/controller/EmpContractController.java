package com.ule.oa.web.controller;

import java.util.HashMap;
import java.util.List;
import java.util.Map;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseBody;

import com.ule.oa.base.po.EmpContract;
import com.ule.oa.base.service.EmpContractService;

/**
  * @ClassName: EmpContractController
  * @Description: 员工合同控制层
  * @author minsheng
  * @date 2017年6月22日 下午7:12:01
 */
@Controller
@RequestMapping("empContract")
public class EmpContractController {
	@Autowired
	private EmpContractService empContractService;
	
	@RequestMapping("/getListByCondition.htm")
	@ResponseBody
	public List<EmpContract> getListByCondition(EmpContract empContract){
		return empContractService.getListByCondition(empContract);
	}
	
	@RequestMapping("/save.htm")
	@ResponseBody
	public Map<String,Object> save(EmpContract empContract){
		Map<String,Object> map = new HashMap<String,Object>();
		
		try{
			empContractService.save(empContract);
			map.put("result", "保存成功!");
			map.put("flag", true);
		}catch(Exception e){
			map.put("msg", e.getMessage());
			map.put("result", "保存失败!");
			map.put("flag", false);
		}
		
		return map;
	}
	@RequestMapping("/updateById.htm")
	@ResponseBody
	public Map<String,Object> updateById(EmpContract empContract){
		Map<String,Object> map = new HashMap<String,Object>();
		String option;
		if(0 == empContract.getDelFlag().intValue()){
			option = "修改";
		}else{
			option = "删除";
		}
		try{
			empContractService.updateById(empContract);
			map.put("result", option+"成功！");
		}catch(Exception e){
			map.put("msg", e.getMessage());
			map.put("flag", false);
			map.put("result", option+"失败！");
		}
		
		return map;
	}
}
