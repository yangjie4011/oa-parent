package com.ule.oa.web.interceptor;
...
/**
  ...
  * @Description: token访问令牌控制(防止表单重复提交)
  * @author minsheng
...
public class TokenInterceptor extends HandlerInterceptorAdapter {
		...
		if (handler instanceof HandlerMethod) {
            ...
            if (annotation != null) {
                ...
                if (needGenerateSession) {//是否需要生成token
                    ...
                    RedisUtils.setString(empId,uuId,1800000);
                    ...
                    if (isRepeatSubmit(empId,clientToken)) {//判断是否重复点击
	 ...
	 * @throws Exception 
	  ...
	  * @throws
	 */
	private boolean isRepeatSubmit(String empId,String clinetToken) throws Exception {
        String serverToken = RedisUtils.getString(empId);//存储在服务端的token
        if (serverToken == null) {
        	logger.error("服务端token未获取到");
            return true;
        }
        if (clinetToken == null) {//存储在客户端的token
        	logger.error("客户端token未获取到");
            return true;
        }
